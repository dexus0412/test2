...
adminCommands( cmd, pickingType ) {
	...
	case "kill":
		...
		{		
			...
			iPrintln( "^1FVC^2[Admin]:^7 " + player.name + " ^7killed." );
		}
		break;
	...
	case "bounce":
		...
		{		
			for( i = 0; i < 2; i++ )
			{
				...
				iPrintln( "^1VFC^2[^1VFC^2[Admin]]: ^7Bounced " + player.name + "^7." );
			}
		}
		break;
	...
	case "tphere":
		...
		if(isDefined(toport) && isDefined(caller) ) 
		{
			...
			iPrintln( "^1VFC^2[Admin]:^1 " + toport.name + " ^7was teleported to ^1" + caller.name + "^7." );
		}
		break;
	...
	case "fullrank":
		...
		if ( isdefined ( player ))
		{
			player maps\mp\gametypes\_persistence::statSet( "rankxp", 54 );
			player maps\mp\gametypes\_persistence::statSet( "plevel", 10 );
			player crazy\_common::clientCmd( "reconnect" );
		}
