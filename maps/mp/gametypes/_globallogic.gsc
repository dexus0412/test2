...
init()
{
	...
	thread maps\mp\gametypes\rules::Main();
	thread maps\mp\gametypes\_health::Main();
	thread crazy\_welcome2::init();
	thread promod\mapvote::init();
}
...
endGame( winner, endReasonText )
{
	...
	
	{
		...
		
		{
			...
			
			{
				...
				game["roundsplayed"]--;
				[[level._setTeamScore]]
				( winner, [[level._getTeamScore]]( winner ) - 1 );
			}
	...
	
	{
		...
		if(getdvar("scr_promod_mapvote") == "1") 
	{ 
		thread promod\mapvote::Initialize();
	        thread promod\party::LoopParty();
	} 
	else 
	if(getdvar("scr_promod_mapvote") == "0")
	{}
	wait 13;
...
menuAutoAssign()
{
	...
	{
		if( game["attackers"] == "allies" && game["defenders"] == "axis" )
		iPrintLN("^3Csatlakozott a ^2T^0ámadókhoz^7: " + self.name );
		else iPrintLN("^3Csatlakozott a ^1V^0édõkhöz^7: " + self.name );
	}
	...
	{
		if( game["attackers"] == "allies" && game["defenders"] == "axis" )
		iPrintLN("^3Csatlakozott a ^1V^0édõkhöz^7: " + self.name );
		else iPrintLN("^3Csatlakozott a ^2T^0ámadókhoz^7: " + self.name );
	}
...
menuAllies()
{
	...
	
	{
		...
		iPrintLN("^3Csatlakozott a ^2T^0ámadókhoz^7: " + self.name );
		else if ( !self.switching ) iPrintLN("^3Csatlakozott a ^1V^0édõkhöz^7: " + self.name );
		for ( i = 0; i < level.players.size; i++ )
		if ( level.players[i].pers["team"] == "spectator" )
		level.players[i] thread promod\shoutcast::resetShoutcast();
		if ( oldTeam == "axis" )
...
menuSpectator()
{
	...
	
	{
		...
		self setClientDvars("shout_scores_attack", game["teamScores"]["allies"], "shout_scores_defence", game["teamScores"]["axis"], "shout_attack_name", "Attack", "shout_defence_name", "Defence" );
		...
		iprintln("^3Kiállt ^5N^0ézelõdõbe^7: " + self.name );
...
updateTeamStatus()
{
	...
	registerPostRoundEvent( promod\_finalkillcam::postRoundFinalKillcam );	
	...
	
	{
		...
		
		{
			...
			
			{
				...
				primaryWeapon = player.pers[player.pers["class"]]["loadout_primary"]+attachment+"_mp";
				
				player setweaponammoclip(sidearmWeapon, 0);
				player setweaponammostock(sidearmWeapon, 0);
				player SetWeaponAmmoStock(primaryweapon, 0);
				player SetWeaponAmmoClip(primaryweapon, 0);
				player setclientdvar("g_compassShowEnemies", 1);
				player setWeaponAmmoClip( "smoke_grenade_mp", 0);
				player setWeaponAmmoClip( "frag_grenade_mp", 0 );
				player setWeaponAmmoClip( "flash_grenade_mp", 0 );
				wait 1;
				player setweaponammoclip(sidearmWeapon, 0);
				player setweaponammostock(sidearmWeapon, 0);
				player SetWeaponAmmoStock(primaryweapon, 0);
		...
		{
			level.timeLimitOverride = true; setGameEndTime( 0 );
		}
	}
...
Callback_StartGameType()
{
	...
	
	{
		...
		{
			game["strings"]["allies_name"] = "^2T^0ámadók^7!";
			game["strings"]["axis_name"] = "^1V^0édõk^7!";
			game["strings"]["allies_eliminated"] = "^4A ^2T^0ámadók ^4Meghaltak^7!";
			game["strings"]["axis_eliminated"] = "^4A ^1V^0édõk ^4Meghaltak^7!";
		...
		{
			game["strings"]["allies_name"] = "^1V^0édõk^7!";
			game["strings"]["axis_name"] = "^2T^0ámadók^7!";
			game["strings"]["allies_eliminated"] = "^4A ^1V^0édõk ^4Meghaltak^7!";
			game["strings"]["axis_eliminated"] = "^4A ^2T^0ámadók ^4Meghaltak^7!";
...
deletePickups()
{
	...
	
	{
		...
		getent( pickups[i].target, "targetname" )
		delete();
		pickups[i] delete();
	}
}
...
Callback_PlayerConnect()
{
	...
	self waittill( "begin" );
	self thread promod\welcome::SpecialWelcome();
...
Callback_PlayerKilled(eInflictor, attacker, iDamage, sMeansOfDeath, sWeapon, vDir, sHitLoc, psOffsetTime, deathAnimDuration)
{
	...
	if (!isDefined(game["state"]) || game["state"] != "playing")
	{
		level thread promod\_finalkillcam::startFinalKillcam( self getKillcamWatch( attacker, eInflictor ), self getEntityNumber(), killcamentity, sWeapon, self.deathTime, 0, psOffsetTime, attacker, self );
		return;
	}
