...
CountPlayers()
{
	...
	for(i = 0; i < players.size; i++)
	{
		...
		if((isdefined(players[i].pers["team"])) && (players[i].pers["team"] == "<E.F.T>^5T^4ámadók"))
			allies++;
		else if((isdefined(players[i].pers["team"])) && (players[i].pers["team"] == "<E.F.T> ^5V^4édõk"))
			axis++;
	}
...
trackFreePlayedTime()
{
	...
	for ( ;; )
	{
		if ( game["state"] == "playing" )
		{
			if ( isDefined( self.pers["team"] ) && self.pers["team"] == "allies" && self.sessionteam != "Nézelõdõk" )
			{
				self.timePlayed["allies"]++;
				self.timePlayed["total"]++;
			}
			else if ( isDefined( self.pers["team"] ) && self.pers["team"] == "axis" && self.sessionteam != "Nézelõdõk" )
			{
				self.timePlayed["axis"]++;
				self.timePlayed["total"]++;
			}
