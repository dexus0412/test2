...
updateTeamBalance()
{
	...
	if ( level.teamBalance && (level.roundLimit > 1 || (!level.roundLimit && level.scoreLimit != 1)) )
	{
    	if( isDefined( game["BalanceTeamsNextRound"] ) )
    		iPrintLnbold( "Csapatelosztás a következõ körben!" );
...
balanceTeams()
{
	...
	iPrintLnBold( "Csapatok kiegyenlítése!" );
...
setPlayerModels()
{
	...
	else game["allies_soldiertype"] = alliesCharSet;
	...
	else game["axis_soldiertype"] = axisCharSet;
...
getJoinTeamPermissions( team )
{
	...
	
	{
		...
		if((isdefined(player.pers["team"])) && (player.pers["team"] == team)) teamcount++;
	}
