...
disableBombBag()
{
	if(level.gametype=="sd")
	{
		...
		if(isDefined(visuals))visuals delete();
		if(isDefined(level.sdBomb))level.sdBomb maps\mp\gametypes\_gameobjects::setVisibleTeam("none");
	}
}
timeoutLoop()
{
	if(!isDefined(game["LAN_MODE"])||!game["LAN_MODE"])thread timeoutLeft();
	if(!isDefined(level.ready_up_over))level.ready_up_over=false;
...
timeoutCall()
{
	...
	if(game["promod_timeout_called"])
	{
		if(isDefined(game["promod_timeout_called_by"]))
		{
			if(self==game["promod_timeout_called_by"])
			{
				...
				if(level.gametype=="sd")game[self.pers["team"]+"_timeout_called"]=0;
				game["promod_timeout_called"]=false;
				if(isDefined(level.scorebot)&&level.scorebot)
				{
					timeout_team="";
					if(self.pers["team"]==game["attackers"])timeout_team="attack";
					else if(self.pers["team"]==game["defenders"])timeout_team="defence";
	...
	if(!isDefined(level.strat_over)||level.strat_over)self iprintln("^3Call timeout again to cancel");
	if(isDefined(level.scorebot)&&level.scorebot)
	{
		timeout_team="";
		if(self.pers["team"]==game["attackers"])timeout_team="attack";
	...
	if(level.gametype=="sd")game[self.pers["team"]+"_timeout_called"]=1;
