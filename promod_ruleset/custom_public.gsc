main()
{
	...
	setDvar( "scr_koth_timelimit", 8 ); // [0->] (minutes)
	...
	// class limits
	setDvar( "class_assault_limit", 30 );
	...
	// sniper
	setDvar( "weap_allow_m40a3", 2 );
	setDvar( "weap_allow_remington700", 2 );
	...
	// website
	setDvar( "promod_hud_website", "^5Legion 7 ^2Promod LIVE" ); // (avoid "//" here)

	// messagecenter
	setDvar( "promod_mc_enable", 0 ); // [0-1]
	setDvar( "promod_mc_rs_every_round", 1 ); // [0-1] (restarts messages on round-based gametypes)
	setDvar( "promod_mc_delay", 12 ); // [1->] (default delay in seconds between messages)
	setDvar( "promod_mc_loopdelay", 1 ); // [1->] (delay in seconds until it starting over)
